#!/usr/bin/env python3
"""
pyDial - AI-powered phone call assistant
...
class pyDial:
    ...
    def setup_routes(self):
        ...
        def ai_agent():
            ...
            # Debug: Print all form data to understand what SignalWire sends
            ...
            if speech_result:
                ...
                if should_end_call:
                    ...
                    # Continue conversation - say response and wait for next input
                    response.say(ai_response, voice=self.ai_voice)
                    
                    # Listen for another response - ONLY gather, no fallback
                    response.gather(
                        input='speech',
                        timeout=10,
                        speech_timeout='auto',
            ...
            elif request.form.get('CallStatus') == 'initiated' and not speech_result:
                # This is the initial call - generate AI greeting
                ...
                # Use Gather to listen for user response - ONLY gather, no fallback
                response.gather(
                    input='speech',
                    timeout=10,
                    speech_timeout='auto',
                ...
                # This is a timeout or no response - generate fallback and end call
                print("⏰ No response detected, generating fallback")
                
                fallback_response = self.generate_ai_response(
                    user_speech="[No response detected]",
                    user_prompt=user_prompt,
                    caller_context=caller_context + " [User didn't respond, end call politely]",
                    call_sid=call_sid
                )
...
Respond with ONLY the words to speak (no asterisks, no stage directions, no descriptions). Keep it to 1 sentence maximum."""
    ...
    def run_cli(self):
        """Main CLI interface"""
        print("🎵 pyDial - AI Phone Call Assistant")
        print("=" * 40)
...
def main():
    parser = argparse.ArgumentParser(description="pyDial - AI-powered phone call assistant")
    parser.add_argument("--version", action="version", version="pyDial 1.0.0")
    ...
    if missing_vars:
        ...
        for var in missing_vars:
            print(f"   {var}")
        print("\n🔧 To fix this:")
        print("   1. Create config.env with your credentials")
