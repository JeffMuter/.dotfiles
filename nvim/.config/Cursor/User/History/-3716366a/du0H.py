        ...
        def ai_agent():
            ...
            if speech_result:
                ...
                if should_end_call:
                    ...
                    response.say(ai_response, voice=self.ai_voice)
                    response.say("Is there anything else you'd like to discuss?", voice=self.ai_voice)
                    ...
                    response.say("I didn't catch that. Feel free to say goodbye if you'd like to end the call.", voice=self.ai_voice)
                    response.gather(
                        input='speech',
                        timeout=10,
                        speech_timeout='auto',
                    ...
                    # Final fallback
                    response.say("Thanks for the conversation! I'll end the call now. Goodbye!", voice=self.ai_voice)
                    response.hangup()
            else:
                # Initial call - start the conversation
                if user_prompt:
                    response.say(f"Hello! {user_prompt}", voice=self.ai_voice)
                else:
                    response.say("Hello! This is the CallHiss AI assistant. I'm calling to have a conversation with you.", voice=self.ai_voice)
                
                response.say("How are you doing today?", voice=self.ai_voice)
                ...
                # Fallback if no response
                response.say("I didn't hear a response. Are you there? Please say something!", voice=self.ai_voice)
                response.gather(
                    input='speech',
                    timeout=10,
                    speech_timeout='auto',
                ...
                # Final fallback
                response.say("I still can't hear you. I'll end the call now. Please try calling back.", voice=self.ai_voice)
                response.hangup()
