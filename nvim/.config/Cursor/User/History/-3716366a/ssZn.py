...
class CallHiss:
    ...
    def setup_routes(self):
        ...
        def ai_agent():
            ...
            is_timeout = request.form.get('SpeechResult') is None and request.form.get('CallSid') is not None
            ...
            if speech_result:
                ...
                if should_end_call:
                    ...
                    # Only generate fallback if we reach this point (no response detected)
                ...
                # This is a timeout - user didn't respond, generate fallback
                print("⏰ No response detected, generating fallback")
                
                fallback_response = self.generate_ai_response(
                    user_speech="[No response detected]",
                    user_prompt=user_prompt,
                    caller_context=caller_context + " [User didn't respond, try to get their attention or end call politely]",
                    call_sid=call_sid
                )
                ...
                # Only generate fallback if we reach this point (no initial response)
                no_response_prompt = self.generate_ai_response(
                    user_speech="[No initial response from person]",
                    user_prompt=user_prompt,
                    caller_context=caller_context + " [Person didn't respond to initial greeting, end call politely]",
                    call_sid=call_sid
                )
