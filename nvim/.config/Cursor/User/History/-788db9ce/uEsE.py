...
def test_client_initialization(mock_env):
    ...
    assert "/api/laml/2010-04-01/Accounts/test-project-id" in client.base_url
...
def test_make_call_payload(mock_post, client, mock_call_details):
    ...
    assert '<Say voice="Polly.' in twiml  # Check for Polly voice attribute
    assert "<break time=" in twiml  # Check for break tag
    assert mock_call_details.caller_name in twiml  # Check content
    assert mock_call_details.contact_name in twiml
    assert mock_call_details.objective in twiml
    assert mock_call_details.additional_context in twiml
