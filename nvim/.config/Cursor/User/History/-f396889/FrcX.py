...
def make_call(call_details):
    ...
    prompt = f"""
    Caller: {call_details.caller_name}
    Contact: {call_details.contact_name}
    Calling: {call_details.phone_number}
    Objective: {call_details.objective}
    """
    if call_details.additional_context:
    ...
    try:
        ...
        if not api_key:
            console.print("[error]Error: ANTHROPIC_API_KEY environment variable not set[/error]")
            return
            
        # Initialize and use the LLM
        console.print("[info]Loading AI model...[/info]")
        llm = create_model(api_key=api_key)
