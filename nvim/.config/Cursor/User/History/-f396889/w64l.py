...
def make_call(call_details):
    ...
    try:
        ...
        # Get voice preferences from environment or use defaults
        voice_gender = os.environ.get("SIGNALWIRE_VOICE_GENDER", "female")
        voice_locale = os.environ.get("SIGNALWIRE_VOICE_LOCALE", "en-US")
        
        # Get background audio settings if configured
        background_audio = None
        background_url = os.environ.get("SIGNALWIRE_BACKGROUND_AUDIO_URL")
        if background_url:
            background_audio = {
                'url': background_url,
                'volume': int(os.environ.get("SIGNALWIRE_BACKGROUND_VOLUME", "-20"))
            }
            console.print(f"[info]Using background audio: {background_url}[/info]")
            console.print(f"[info]Background volume: {background_audio['volume']} dB[/info]")
            
        # Initialize SignalWire client
        console.print("[info]Initializing SignalWire client...[/info]")
        console.print(f"[info]Using voice: {voice_gender} ({voice_locale})[/info]")
