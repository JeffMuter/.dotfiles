...
class TinyLlamaModel(LLMInterface):
    ...
    def generate_response(self, prompt: str, max_length: Optional[int] = None) -> str:
        ...
        with Progress(
            ...
            # Generate response with reduced parameters for faster generation
            response = self.pipeline(
                formatted_prompt,
                max_length=max_length or 150,  # Reduced from 200
                num_return_sequences=1,
                pad_token_id=self.tokenizer.eos_token_id,
                do_sample=True,
                temperature=0.7,
                top_p=0.9,
                repetition_penalty=1.2,  # Help prevent repetitive text
