...
def make_call_menu() -> CallDetails:
    ...
    if not target_number:
        raise ValueError("TARGET_PHONE_NUMBER environment variable must be set")
    
    # Validate and format the target number
    try:
        phone_number = validate_phone_number(target_number)
    except ValueError as e:
        raise ValueError(f"Invalid TARGET_PHONE_NUMBER in environment: {str(e)}")
