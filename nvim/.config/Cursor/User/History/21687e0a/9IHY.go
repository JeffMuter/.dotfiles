	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.path == "/" && tt.expectedStatus == http.StatusOK {
				body := w.Body.String()
				assert.Contains(t, body, "<!DOCTYPE html>", "Response should contain HTML doctype")
...
func TestRouterErrorHandling(t *testing.T) {
	...
	for _, path := range invalidPaths {
		t.Run("Invalid_path_"+path, func(t *testing.T) {
			...
			// Should either return 404 or be handled by static file server
			assert.True(t, w.Code == http.StatusNotFound || w.Code == http.StatusOK || w.Code == http.StatusForbidden,
