...
Your objective is: {call_details.objective}"""
    ...
    def make_simple_ai_call(self, prompt: str, to_number: Optional[str] = None) -> dict:
        ...
        # Create cXML script with AI configuration
...
<Response>
    <Connect>
        ...
        # Create a temporary cXML bin
        bin_response = requests.post(
            f"{self.base_url}/api/laml/2010-04-01/Accounts/{self.project_id}/LaML/Bins",
            headers={'Content-Type': 'application/x-www-form-urlencoded'},
            auth=(self.project_id, self.api_key),
            data={'Content': xml_script}
        )
        bin_response.raise_for_status()
        bin_data = bin_response.json()
        bin_url = bin_data.get('url')
        
        if not bin_url:
            raise RuntimeError("Failed to create cXML bin")
