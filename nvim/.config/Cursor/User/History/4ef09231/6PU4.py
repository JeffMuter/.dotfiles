...
This version uses direct SWML configuration instead of webhooks.
"""
import os
import requests
...
class SignalWireClient:
    ...
    def initiate_ai_call(self, to_number: Optional[str] = None) -> dict:
        """Initiate an AI call using direct SWML configuration.
        ...
        # Direct SWML configuration for the AI agent
        swml = f'''<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <AI engine="openai" 
        voice="{os.environ.get('SIGNALWIRE_VOICE_GENDER', 'female')}-{os.environ.get('SIGNALWIRE_VOICE_LOCALE', 'en-US')}-Neural2-D"
        ...
        <Prompt 
            ...
            {os.environ.get('AI_PROMPT', 'You are a helpful AI assistant making a phone call.')}
            Remember to:
        ...
        payload = {
            ...
            'Twiml': swml,  # Direct SWML configuration
        }
