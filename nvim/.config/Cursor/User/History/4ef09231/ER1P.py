...
Your objective is: {call_details.objective}"""
    ...
    def make_simple_ai_call(self, to_number: str, prompt: str) -> dict:
        """Make a simple AI call using LaML and webhooks.
        ...
        webhook_url = os.environ.get("SIGNALWIRE_WEBHOOK_URL")
        if not webhook_url:
            raise ValueError("SIGNALWIRE_WEBHOOK_URL environment variable must be set")

        # API endpoint for LaML calls
        url = f"https://{self.space_name}.signalwire.com/api/laml/2010-04-01/Accounts/{self.project_id}/Calls"
        ...
        # Request body - encode prompt in webhook URL
        data = {
            'From': self.phone_number,
            'To': to_number,
            'Url': f"{webhook_url}?prompt={urlencode({'prompt': prompt})['prompt='.len():]}",
            'Method': 'POST'
        }
