...
class SignalWireClient:
    ...
    def make_simple_ai_call(self, prompt: str, to_number: Optional[str] = None, 
        ...
        # Create LaML XML that matches the Node.js structure
        ai_params = f'voice="{voice}" confidence="{confidence}" frequencyPenalty="{frequency_penalty}"'
...
<Response>
    <Connect>
        ...
        # Create call payload - use direct XML in Url parameter
        payload = {
            'To': target_number,
            'From': self.phone_number,
            'Url': f"data:application/xml;charset=utf-8,{requests.utils.quote(xml_script)}"
        }
