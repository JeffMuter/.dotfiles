    ...
    def _check_call_status(self, call_sid: str, max_retries: int = 10, delay: int = 2) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Only print status updates when the status changes
                if attempt == 0 or status_data.get('status') != self._last_status:
                    print(f"\nCall Status: {status_data.get('status')}")
...
6. Be sure to hang up the call at the end of every conversation
        ...
        try:
            ...
            # Initialize last status for status tracking
