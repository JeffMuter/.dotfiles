...
class SignalWireClient:
                        ...
                        "steps": [
                            {
                                ...
                                "text": prompt_text,
                                "step_criteria": "Initial contact with the person",
                                "functions": [],
                                "valid_contexts": ["Initial Contact"],
            ...
            "languages": [
                {
                    "name": "English",
                    "code": "en-US",
                    "function_fillers": ["okay", "understood"],
                    "speech_fillers": ["um", "hmm"]
                }
            ]
        }
    ...
    def make_call(self, call_details: CallDetails) -> dict:
        ...
        # Add required phone numbers at root level
        ai_config.update({
        ...
        try:
            response = requests.post(
                f"{self.base_url}",
                json=ai_config,  # Send the config directly as the payload
                headers=headers,
