...
<Response>
    <Connect>
        <AI voice="{voice}">
            <Prompt confidence="{confidence}" frequencyPenalty="{frequency_penalty}">
                {prompt}
            </Prompt>
        </AI>
    </Connect>
        ...
        # URL encode the XML
        encoded_xml = quote(xml_script)
        
        # Create call payload
        payload = {
            'To': target_number,
            'From': self.phone_number,
            'Url': f"data:application/xml;base64,{base64.b64encode(xml_script.encode()).decode()}"
        }
