...
class SignalWireClient:
    ...
    def __init__(self, api_key: Optional[str] = None, voice_gender: str = 'female', 
                 voice_locale: str = 'en-US', background_audio: Optional[dict] = None):
        ...
        Args:
            ...
            background_audio: Optional dict with background audio settings:
                {
                    'url': str,  # URL to audio file
                    'volume': int # Volume level in dB (default: -20)
                }
        """
        ...
        # Note: We'll construct the LAML dynamically in make_call to handle background audio
...
{content}
    ...
    def make_call(self, call_details: CallDetails) -> dict:
        ...
        # Create LAML content based on whether we have background audio
        if self.background_audio:
            laml_content = f"""    <Play volume="{self.background_audio.get('volume', -20)}">
        {self.background_audio['url']}
    </Play>
    <Say voice="Polly.{self.voice_name.lower()}">
        <break time="500ms"/>
        {prompt}
    </Say>"""
        else:
            laml_content = f"""    <Say voice="Polly.{self.voice_name.lower()}">
        <break time="500ms"/>
        {prompt}
    </Say>"""
        
        # Create final LAML
        laml = self.default_laml.format(content=laml_content)
