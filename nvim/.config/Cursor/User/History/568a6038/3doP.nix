...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    echo "4. Start ngrok (Terminal 1): ./start_ngrok.sh"
    echo "5. Run application (Terminal 2): ./run.sh"
    echo ""
    echo "📚 Configuration:"
    echo "   All environment variables are defined in config.env"
    echo "   Use './run.sh' to ensure proper virtual environment activation"
    echo "   Use './start_ngrok.sh' to automatically set up ngrok tunnel"
    echo ""
